...
class weiboLogin:
    ...
    def getServerData(self):
        url = 'http://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su=ZW5nbGFuZHNldSU0MDE2My5jb20%3D&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.19)&_=1442991685270'
        data = urllib2.urlopen(url).read()
        # p = re.compile(r'(.∗?)',re.S)
        p = re.compile(r'sinaSSOController.preloginCallBack\((.*?)\)', re.S)
        try:
    ...
    def login(self, username, psw):
        ...
        cookie=cookie_tool.initCookies()
        ...
        #p = re.compile(r'location\.replace\(\"(.*?)\"\);')
        try:
            login_url = p.search(text)
            if login_url==None:
                login_url='https://passport.weibo.com/wbsso/login?ssosavestate=1561564335&url=http%3A%2F%2Fweibo.com%2Fajaxlogin.php%3Fframelogin%3D1%26callback%3Dparent.sinaSSOController.feedBackUrlCallBack&display=0&ticket=ST-MzYzODc3MTczNQ==-1530028335-tc-FBA108AB7BF61FAF99961CAA06D82606-1&retcode=0'
            else:
    ...
    #cookie登录
    def loginbycookies(self, username, psw):
        cookie_tool=CookieTools()
